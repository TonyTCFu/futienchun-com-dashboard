...
- Workspace 提交 `7598e80`、部署仓库提交 `7da542d` 已推送；公网首页正文命中 `2026-07-30 / 2026-07-30`，`/healthz=ok`，`/version.json`、首页 `ETag` 与 `X-Dashboard-Version` 一致为 `442a8c27c8a50974d06edf7d95d6fa74f0e3c02e71a5c2fb03e8f5222b4463a8`。
...
- `.github/workflows/tw-dashboard-daily.yml` 已改为云端允许直接读取 TWSE 公开收盘资料，并在失败时写入 Step Summary；生成页面的更新日期和行情/回测最新日期不等于当天时，验收仍会失败。
- 本机正式补跑已推进 Dashboard 与行情/回测序列到 `2026-07-28`；公网发布尚待部署仓库同步后复核。
...
- 2026-07-27 已确认今天漏更不是缓存问题：台湾股市交易日，本地和公网补跑前仍停在 `2026-07-26 / 2026-07-24`，公网 `/healthz` 与版本 header 正常但 hash 为旧版 `e9ff3ce42c50092935e5715a6049e1cadfbc7020b5b26bb66880ebcb76bcdc47`。
- 2026-07-27 已按正式 public-close 口径补跑，Dashboard 更新日期、行情/回测最新日、模型盘市值日均推进到 `2026-07-27`；市值档 `quote_count=13`、`missing_count=0`，持仓市值 `NT$252,795.73`，未实现盈亏 `NT$-3,789.97`。
- 2026-07-27 本地模拟盘自动落账 `3` 笔：`006208` 卖出 `13` 股、`00881` 卖出 `49` 股、`2330` 买入 `1` 股；Dashboard 显示已落账模拟成交 `3` 笔、待自动落账 `0` 笔。
- 2026-07-27 已新增云端日更防线 `.github/workflows/tw-dashboard-daily.yml`，让部署仓库在 GitHub Actions 上按台湾交易日历自动重建、验证并 commit/push，触发 Render 部署。该链路解决“本机 Codex cron 空跑/断线/睡眠导致漏更”的根因；本机 automation 后续只作为补偿与审计。
- 2026-07-27 已补强 Render 进程内 rebuild 命令，加入 `--start 2024-01 --offline-cache --allow-stale-cache --model-invest-ratio 0.75` 且不写死 `--end`，避免跨月停在旧窗口。
- 2026-07-26 用户明确规则：台湾股市只要有开盘，当天 Dashboard 就必须完成日更；只有周末或交易所正式休市日才跳过。已更新 `dashboard` 主日更与 `dashboard-4` 补偿自动化 prompt，要求开盘日完成重建、提交、推送公网和缓存版本验收，并保持正常成功不向项目对话框发送完整摘要。
- 2026-07-26 已按用户截图完成漏跑补偿：`2026-07-26` 为周日，正确补跑目标是最近可用交易日 `2026-07-24`；本轮保持只读公开行情与本地模拟盘边界，未读取敏感配置，未调用券商下单。
- 2026-07-26 Dashboard 已推进到更新日期 `2026-07-26`、行情/回测最新日 `2026-07-24`、模型盘市值日 `2026-07-24`；市值档 `quote_count=13`、`missing_count=0`，持仓市值 `NT$259,605.29`，未实现盈亏 `NT$-5,193.50`。
- 2026-07-26 本地模拟盘自动落账 `2` 笔卖出：`006208` 卖出 `18` 股、`00881` 卖出 `66` 股；策略监控 `signal-pill sell=0`，待自动落账 `0` 笔。
- 2026-07-26 Workspace 提交 `c2c03e6` 已推送 `origin/main`，部署仓库提交 `aa62023` 已推送 `futienchun-com-dashboard/main`；公网首页正文命中 `2026-07-26 / 2026-07-24` 与 KPI Summary，`/healthz=ok`，`/version.json`、首页 `X-Dashboard-Version` 与 weak `ETag` 一致为 `e9ff3ce42c50092935e5715a6049e1cadfbc7020b5b26bb66880ebcb76bcdc47`。
- 2026-07-26 用户再次明确：每日收盘更新与漏跑补偿的正常结果不要发进项目对话框；只有错误、阻塞、公网旧版、发布失败、验证失败或需要用户决策时才通知。详细结果进入 Dashboard “每日更新 Summary”、项目交接文档与 automation memory。
- 2026-07-26 Dashboard “每日更新 Summary” 已扩展为量化模型基金组合每日摘要：展示模型盘未实现损益率、滚动回测净值、年化报酬、最大回撤、年化波动、平均换手、AI 风险差与模拟盘执行状态，并和普通协方差或策略基线对照。
...
- 2026-07-20 Dashboard 首页与第二屏已压缩：首屏优先显示 `今日状态`，手机端摘要卡片两列、资产池横向 chip；`调仓日历` 改为 6 项 timeline 与一条模拟盘执行摘要。按钮与折叠区统一为“模拟盘状态”，不再出现“执行检查/执行复核”。
...
- 2026-07-17 本地模拟盘自动落账新增成交 `0` 笔；`data/simulated_trades_2026-07-17.csv` 仅含表头，持仓 latest 维持 `13` 檔。
- 2026-07-17 策略监控有 `1` 笔待复核买入：`00919` 买入 `96` 股；卖出 `0` 笔，`signal-pill sell=0`、可见 `建议卖出=0`。
- 2026-07-17 Dashboard 研究摘要 QA 基线为 `AI 供应链权重 31.95%`、`风险贡献 51.69%`、`风险-权重差 +19.74%`，调仓状态为“本轮有 1 笔待复核调仓：买入 1 笔、卖出 0 笔”；iCloud Obsidian `台股量化基金.md` 已同步。
- 2026-07-17 Workspace 提交 `f0ef2d6` 已推送 `origin/main`，部署仓库提交 `84415fc` 已推送 `futienchun-com-dashboard/main`；Render 首页正文已验证切到 `2026-07-17 / 2026-07-17`，`/healthz=ok`，`/version.json` 与首页 `ETag` / `X-Dashboard-Version` 一致为 `6f5675aae69902f9de0b675180f09925014f7ee60d7fa3cef8e4b75067360395`。
- 2026-07-16 已按 TWSE 官方 2026 年休市 API 确认交易日，并完成收盘日更；本轮未读取 `.env`、`.shioaji.local.env`、API key 或 token，未调用券商下单。
- 2026-07-16 首次 public-close 重建时，页面日期已切到 `2026-07-16` 但行情/回测共同日仍停在 `2026-07-13`；直接查询 TWSE `STOCK_DAY` 确认 7/16 收盘价已发布后，补刷了落后的 `00713`、`00881`、`00919`、`2330`、`2317`、`2303`、`2881`、`1301` 月缓存。
- 2026-07-16 第二次正式重建已推进到 `行情/回测序列最新日期：2026-07-16`；生成 `data/model_portfolio_market_2026-07-16.csv`，`quote_count=14`、`missing_count=0`，当前持仓市值 `NT$276,726.00`、未实现盈亏 `NT$-1,221.31`。
...
- 2026-07-02 Workspace 日更提交已推送到 `origin`，部署仓库日更提交已推送到 `futienchun-com-dashboard`；Render 首页正文已验证切到 `2026-07-02 / 2026-07-02`，公网 `signal-pill sell=0`、可见 `建议卖出=0`。
...
- 2026-06-23 已按用户要求刷新今日 Dashboard：完整 public-close 重建耗时 `real 72.19`，页面今日更新日期切到 `2026-06-23`，但正式行情/回测序列最新日期仍为 `2026-06-22`，未生成 `data/model_portfolio_market_2026-06-23.csv`；这是 TWSE public-close 数据未共同前进，不是页面生成失败。
- 2026-06-23 本轮模拟盘继续幂等：最后模拟盘执行日仍为 `2026-06-22`，已落账模拟成交仍为 `3` 笔，`signal-pill sell=0`、`建议卖出=0`；本地 QA 汇总通过，Obsidian `台股量化基金.md` 已同步研究摘要。
- 2026-06-24 已完成每日收盘自动化：完整 public-close 重建耗时 `real 35.97`，页面今日更新日期切到 `2026-06-24`，但公开收盘价共同交易日仍停在 `2026-06-23`，未生成 `data/model_portfolio_market_2026-06-24.csv`；`data/model_portfolio_market_2026-06-23.csv` 为本轮刷新产物，15 檔成功、缺失 0 檔。
- 2026-06-24 本轮模拟盘继续幂等：新增模拟成交 `0` 笔，最后模拟盘执行日仍为 `2026-06-23`，已落账模拟成交 `3` 笔，其中卖出 `3` 笔；当前 `2317` 剩 `11` 股、`2881` 剩 `120` 股、`2882` 剩 `124` 股；`signal-pill sell=0`、可见 `建议卖出=0`。
- 2026-06-24 调仓日历为：最后回测调仓日 `2026-06-17`，预计下次回测调仓 `2026-06-29`，距下次还差 `4` 个共同交易日；研究摘要 QA 基线同步为 `34.38% / 49.89% / +15.51% / trade_count=3`，本地 QA 汇总通过。
- 2026-06-25 已完成每日收盘自动化：完整 public-close 重建耗时 `real 29.43`，页面今日更新日期切到 `2026-06-25`，公开收盘价共同交易日推进到 `2026-06-24`，正式生成 `data/model_portfolio_market_2026-06-24.csv`；本轮需带 `PYTHONPATH=$HOME/Library/Python/3.9/lib/python/site-packages` 复用 user-site `scipy`。
- 2026-06-25 本轮新增本地模拟卖出 `2` 笔：`2317` 卖出 `3` 股、`2881` 卖出 `24` 股；执行后 `2317` 剩 `8` 股、`2881` 剩 `96` 股、`2882` 维持 `124` 股；`signal-pill sell=0`、可见 `建议卖出=0`。
- 2026-06-25 调仓日历为：最后回测调仓日 `2026-06-17`，预计下次回测调仓 `2026-06-29`，距下次还差 `4` 个共同交易日；研究摘要 QA 基线同步为 `33.00% / 52.71% / +19.71% / trade_count=2`，本地 QA 汇总通过；公网 Render 正文已验证切到 `2026-06-25` / `2026-06-24`。用户已确认 `origin/main` 的 Antigravity/300k TWD 分叉属于另一个智能体维护，后续 Dashboard 日更默认只推公网部署用的 `dashboard` remote，不强推、不自动合并 `origin`。
- 2026-06-26 已完成每日收盘自动化：完整 public-close 重建耗时 `real 26.48`，页面今日更新日期切到 `2026-06-26`，公开收盘价共同交易日推进到 `2026-06-25`，正式生成 `data/model_portfolio_market_2026-06-25.csv`；本轮继续带 `PYTHONPATH=$HOME/Library/Python/3.9/lib/python/site-packages` 复用 user-site `scipy`。
- 2026-06-26 本轮新增本地模拟卖出 `3` 笔：`2317` 卖出 `2` 股、`2303` 卖出 `27` 股、`2881` 卖出 `20` 股；执行后 `2317` 剩 `6` 股、`2303` 剩 `107` 股、`2881` 剩 `76` 股、`2882` 维持 `124` 股；`signal-pill sell=0`、可见 `建议卖出=0`。策略监控仍有 `2454` 一笔待确认买入 `1` 股。
- 2026-06-26 调仓日历为：最后回测调仓日 `2026-06-17`，预计下次回测调仓 `2026-06-29`，距下次还差 `3` 个共同交易日；研究摘要 QA 基线同步为 `33.00% / 52.63% / +19.63% / trade_count=1`，本地 QA 汇总通过。
- 2026-06-27 已完成每日收盘自动化：完整 public-close 重建耗时 `real 27.71`，页面今日更新日期切到 `2026-06-27`，公开收盘价共同交易日推进到 `2026-06-26`，正式生成 `data/model_portfolio_market_2026-06-26.csv`；本轮继续带 `PYTHONPATH=$HOME/Library/Python/3.9/lib/python/site-packages` 复用 user-site `scipy`。
- 2026-06-27 本轮新增本地模拟卖出 `1` 笔：`2317` 卖出 `2` 股；执行后 `2317` 剩 `4` 股、`2303` 剩 `107` 股、`2881` 剩 `76` 股、`2882` 维持 `124` 股；`signal-pill sell=0`、可见 `建议卖出=0`。策略监控仍有 `2454` 一笔待确认买入 `1` 股。
- 2026-06-27 调仓日历为：最后回测调仓日 `2026-06-17`，预计下次回测调仓 `2026-06-29`，距下次还差 `2` 个共同交易日；研究摘要 QA 基线仍为 `33.00% / 52.63% / +19.63% / trade_count=1`，Obsidian 项目卡片已同步最新结构变化行。
- 2026-06-27 本地提交 `b066ca9 chore: refresh dashboard for 2026-06-27` 已创建，但 `dashboard` 与 `origin` 两个 HTTPS 远端推送均被本机 GitHub 凭证阻塞：`could not read Username for 'https://github.com': Device not configured`；Render `/healthz` 为 `200`，但首页正文仍停在 `2026-06-26 / 2026-06-25`，因此公网尚未更新。
- 2026-06-28 已完成每日收盘自动化：完整 public-close 重建耗时 `real 23.88`，页面今日更新日期切到 `2026-06-28`；今天是周日，公开收盘价共同交易日仍为 `2026-06-26`，本轮刷新 `data/model_portfolio_market_2026-06-26.csv`。
- 2026-06-28 本轮 `--execute-simulated-trades` 对 `2026-06-26` 保持幂等，未新增 dated 模拟成交文件；最后模拟盘执行日仍为 `2026-06-26`，已落账 `2317` 卖出 `2` 股，执行后 `2317` 剩 `4` 股；`signal-pill sell=0`、可见 `建议卖出=0`，策略监控仍有 `2454` 一笔待确认买入 `1` 股。
- 2026-06-28 调仓日历为：最后回测调仓日 `2026-06-17`，预计下次回测调仓 `2026-06-29`，距下次还差 `1` 个共同交易日；研究摘要 QA 基线同步为 `33.00% / 52.82% / +19.82% / trade_count=1`，本地 QA 汇总通过。
- 2026-06-28 本地提交已创建；初始 `dashboard` 与 `origin` 两个 HTTPS 远端推送被本机 GitHub 凭证阻塞：`could not read Username for 'https://github.com': Device not configured`。
- 2026-06-29 已修复公网推送链路：本机没有 `gh` 与 Git credential helper，但 `~/.ssh/config` 里的 `github-worldcup` alias 可认证 GitHub；已把 `dashboard` remote 的 push URL 改为 `git@github-worldcup:TonyTCFu/futienchun-com-dashboard.git` 并成功推送 `72df915..53b6dcc`。Render 首页正文已验证切到 `今日 Dashboard 更新日期：2026-06-28`、`行情/回测序列最新日期：2026-06-26`，且 `signal-pill sell=0`。`origin/main` 仍是 Antigravity/资金规模相关并行分叉，不强推、不自动合并。
- 2026-06-29 已完成每日收盘自动化：完整 public-close 重建耗时 `real 25.75`，页面今日更新日期切到 `2026-06-29`，但公开收盘价共同交易日仍为 `2026-06-26`，本轮刷新 `data/model_portfolio_market_2026-06-26.csv`。
- 2026-06-29 本轮模拟盘继续幂等：新增模拟成交 `0` 笔，最后模拟盘执行日仍为 `2026-06-26`，已落账 `2317` 卖出 `2` 股，执行后 `2317` 剩 `4` 股；`signal-pill sell=0`、可见 `建议卖出=0`，策略监控仍有 `1` 笔待确认买入。
- 2026-06-29 调仓日历为：最后回测调仓日 `2026-06-17`，预计下次回测调仓 `2026-06-29`，距下次还差 `1` 个共同交易日；研究摘要 QA 基线维持 `33.00% / 52.82% / +19.82% / trade_count=1`，本地 QA 汇总通过。
- 2026-06-29 提交 `c82fe94` 已推送到 `dashboard` 远端，Render 首页正文第 2 轮验证切到 `今日 Dashboard 更新日期：2026-06-29` 与 `行情/回测序列最新日期：2026-06-26`；`origin` 普通推送仍因 HTTPS 凭证失败且存在并行分叉，本轮未强推、未自动合并。
...
- 2026-07-27 Dashboard 首屏已改为基金收盘后状态：实际持仓市值、模拟盘现金、基金总资产、累计盈亏、今日策略形成原因与 Portfolio KPI 直接可见；热力图、回测、组合明细、数据问题和策略监控默认收起。
- 2026-07-27 已移除重复的右侧模型盘建仓栏；当前持仓明细继续保留代码、名称、当前价、股数、当前市值和个股盈亏。桌面与移动端浏览器验收通过。
