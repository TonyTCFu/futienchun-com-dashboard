...
- Dashboard 首页优先呈现紧凑的 `今日状态`、模型盘市值、已落账成交和待自动落账数量；手机端资产池使用横向 chip，调仓日历使用 timeline，避免大卡片只显示少量文字。
...
需要特别区分两件事：历史价格只用于分析选股策略与目标权重，真正建仓价格必须使用今日收盘价、开盘价或明确录入的模拟成交价。价格尚未入账时，模型盘只输出目标权重和目标金额，`execution_price`、`shares`、`market_value` 会留空，并在仪表盘中显示“待收盘价入账”。等价格可用后，再用目标金额除以价格换算整数股；支持零股，但股数必须为整数。
...
仪表盘顶部的“模拟盘状态”按钮会展开状态面板，检查建仓股数、整数股规则、资金使用、买进税费口径、今日盈亏和后续调仓节奏。这个按钮只做本地页面审计，不会连接券商或送出订单，也不会阻挡 `--execute-simulated-trades` 自动写入本地模拟成交 CSV。
...
The public Dashboard is the report surface. Normal successful daily close, non-trading-day, review, or miss-compensation results should not be sent back into the project Codex thread. The thread should only receive errors, blockers, stale public versions, failed publishing, failed verification, or user-decision needs.

The Dashboard daily update summary must include a model-fund portfolio KPI block: model-portfolio P&L, rolling backtest net value, annual return, max drawdown, annual volatility, turnover, AI risk gap, and paper-trade execution status, with comparisons against the plain covariance or policy baseline where available.
...
仪表盘中的“模拟盘自动执行记录”区块只显示尚未写入本地 CSV 的待自动落账买入或卖出建议。2026-06-03 初始建仓单按虚拟盘成交口径视为已执行，不再作为待处理订单显示；对应持仓数量、成本和盈亏会在“今日持仓与收盘盈亏”区块展示。页面按钮只更新当前浏览器的本地审计状态，不会连接券商、不具备真实交易能力。每笔待自动落账建议会显示一个稳定单号（`trade_id`），用于对齐页面审计、脚本落账和本地模拟成交 CSV；单号不包含价格、股数、中文原因或任何券商凭证。

若要把本轮建议单真正落入模拟盘，使用 `--execute-simulated-trades`。脚本会在本地现金/持仓约束允许时同时处理买入与卖出，写入 `data/simulated_trades_交易日.csv`，并更新 `data/simulated_positions_latest.csv` 与 `data/simulated_positions_交易日.csv`；后续仪表盘会优先读取最新模拟持仓，而不是重复使用初始建仓单。新写入的模拟成交 CSV 会包含 `trade_id` 欄位；旧 CSV 没有此栏位时，脚本仍会用交易日、标的和方向做兼容去重。现在每日收盘后的自动重建流程也会带上这个开关，所以 Dashboard 会同步刷新持仓、执行状态与研究说明区。
...
Dashboard 的“模拟盘自动执行记录”区块会显示批次状态小结：目前批次、已写入本地模拟成交 CSV 的批次、旧格式纪录与目前待自动落账批次。旧模拟成交 CSV 若没有 `trade_id`，会单独显示为“旧格式”，并继续按交易日、标的和方向相容防重；页面审计只保存在当前浏览器，真正写入本地模拟成交 CSV 仍需执行 Python 主脚本。

若独立加权指数接口本轮未返回，Dashboard 不应显示大块“暂不可用”；页面会改用模型盘持仓市值、未实现盈亏与已落账模拟成交作为首屏摘要。持仓价格可以来自 Shioaji 只读行情或公开收盘价市值档，指数接口失败不应影响模型盘市值、盈亏、模拟成交和策略监控的可读性。
