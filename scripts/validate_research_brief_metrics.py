...
EXPECTED_METRICS = {
    "ai_weight_percent": "30.90%",
    "risk_contribution_percent": "50.76%",
    "risk_weight_gap_percent": "+19.87%",
    "trade_status": "settled_5",
}
...
def validate_metrics(dashboard_path: Path) -> dict[str, str]:
    ...
    patterns = {
        ...
        "trade_status": r"本轮没有新的待自动落账调仓；已有 5 笔本日模拟调仓转为观察",
    }
    ...
    for key, pattern in patterns.items():
        ...
        if not match:
            missing.append(key)
            continue
        actual[key] = match.group(1) if match.groups() else EXPECTED_METRICS[key]
