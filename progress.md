...
- 已按 TWSE 官方 2026 年市场开休市资料判定 `2026-07-27` 为周一且不在休市清单内，按交易日强制更新处理；本轮未读取 `.env`、`.shioaji.local.env`、API key、token 或凭证文件，未调用券商下单、改单、撤单或账户交易接口。
- 已确认本地与公网在补跑前仍停在 Dashboard 更新日期 `2026-07-26`、行情/回测最新日 `2026-07-24`；公网 `/healthz=ok` 且 `/version.json`、首页 `X-Dashboard-Version` 与 `ETag` 均为旧版本 `e9ff3ce42c50092935e5715a6049e1cadfbc7020b5b26bb66880ebcb76bcdc47`，说明不是浏览器缓存，而是 7/27 交易日内容没有生成/发布。
- 已执行正式 public-close 补跑：`--start 2024-01 --end 2026-07 --offline-cache --allow-stale-cache --model-portfolio --model-build-date 2026-06-03 --model-invest-ratio 0.75 --model-method multi-factor-shrink --ai-tilt moderate --market-source public-close --market-mode close --execute-simulated-trades`，用时 `real 26.23`。
- Dashboard 已推进为：今日 Dashboard 更新日期 `2026-07-27`，行情/回测序列最新日期 `2026-07-27`，模型盘市值日 `2026-07-27`。
...
- 已按用户要求把日更规则收紧为：台湾股市只要有开盘，当天 Dashboard 就必须更新；只有周六、周日或交易所正式休市日才跳过。
- 已更新 Codex automation `dashboard`：每日 13:45 收盘后执行，开盘日必须完成重建、提交、推送公网和缓存版本验收。
- 已更新 Codex automation `dashboard-4`：工作日 16:30 复核主日更；若开盘日未落地或公网仍旧，必须立即按正式日更流程补偿。
- 已保持通知边界：正常成功/复核通过/非交易日跳过不向项目对话框发送完整摘要，只有异常、失败、阻塞、公网旧版或需要 Tony 决策时通知。

### Verification Log

- 已通过 Codex automation update 工具确认 `dashboard` 与 `dashboard-4` 均已更新并保持 `ACTIVE`。
...
- 最终正式重建已取得 `2026-07-13` 公开收盘价；Dashboard 快照时间更新为 `2026-07-13T22:11:53`，行情/回测最新日为 `2026-07-13`，正式刷新 `data/model_portfolio_market_2026-07-13.csv` 与 summary，14 檔成功、缺失 0 檔。
- 虚拟盘已自动落账 `2026-07-13` 模拟成交 `3` 笔：`2317` 卖出 1 股、`2454` 卖出 1 股、`2412` 卖出 164 股；执行后待复核调仓 `0` 笔，`signal-pill sell=0`、可见 `建议卖出=0`。
- 已把 Dashboard 视觉改成更柔和的浅色工作台，并把“模拟盘调仓确认 / 手动复核”语义改为“模拟盘自动执行记录 / 页面复核审计”；本地虚拟盘由 `--execute-simulated-trades` 自动写 CSV，页面按钮只记录浏览器复核状态，不连接券商、不送出实盘订单。
- 已同步 Dashboard 研究摘要漂移到 QA 基线与 iCloud Obsidian `台股量化基金.md`：`AI 供应链权重 32.55%`、`风险贡献 52.44%`、`风险-权重差 +19.89%`、`trade_count=2`。
...
- 页面解析通过：`更新日期=2026-07-13`、`快照时间=2026-07-13T22:11:53`、`回测最新日=2026-07-13`、`本日模拟成交=3 笔`、`待复核调仓=0 笔`、`signal-pill sell=0`、可见 `建议卖出=0`。
- `PYTHONPATH=$HOME/Library/Python/3.9/lib/python/site-packages python3 scripts/run_local_qa_checks.py --skip-dashboard-fixture` 通过，输出 `/tmp/tw_quant_local_qa_summary.md` 与 `/tmp/tw_quant_local_qa_summary.json`。
- Playwright 视觉验证通过：桌面 `1440x1100` 与手机 `390x844` 均可渲染；手机 `docWidth=390/clientWidth=390`，控制台 `0 errors / 0 warnings`。
...
- 已确认运行路径为 `/Users/tonyfu/Library/Mobile Documents/iCloud~md~obsidian/Documents/Codex/projects/台股_稳健投资组合量化模型构建`，未读取 `.env`、`.shioaji.local.env`、API key、token，未调用券商下单。
- 新 Workspace 仍没有 `.venv/bin/python`，本轮继续使用系统 `python3` 并带 `PYTHONPATH=$HOME/Library/Python/3.9/lib/python/site-packages`，确保 `multi-factor-shrink` 使用既有 user-site 依赖。
- 已执行正式重建，耗时 `real 20.15`；Dashboard 今日更新日期切到 `2026-07-02`，公开收盘价共同序列推进到 `2026-07-02`，生成 `data/model_portfolio_market_2026-07-02.csv` 与 summary，15 檔成功、缺失 0 檔。
- 本轮 `--execute-simulated-trades` 新增本地模拟成交 `2` 笔，均为卖出：`2317` 卖出 `1` 股、`2303` 卖出 `22` 股；执行后 `2317` 剩 `2` 股、`2303` 剩 `85` 股。
- 策略监控显示待确认调仓 `0` 笔；`signal-pill sell=0`、可见 `建议卖出=0`，没有已落账标的仍显示红色建议卖出。
- 已同步 Dashboard 研究摘要漂移到 iCloud Obsidian `台股量化基金.md`：策略结构变化行更新为 `HHI 0.1609`、前三大权重 `61.92%`、有效持仓数 `6.22`、结构变化风险贡献 `52.78%`。
- Workspace 日更提交已推送到 `origin`；部署仓库日更提交已推送到 `futienchun-com-dashboard`。
- 公网验证通过：`https://futienchun-com-dashboard.onrender.com/` 首页正文第 2 轮轮询命中 `今日 Dashboard 更新日期：2026-07-02` 与 `行情/回测序列最新日期：2026-07-02`，公网 `signal-pill sell=0`、可见 `建议卖出=0`，`/healthz` 返回 `200`。

### Verification Log

- `PYTHONPATH=$HOME/Library/Python/3.9/lib/python/site-packages python3 -m py_compile src/risk_dashboard.py scripts/serve_dashboard.py scripts/run_local_qa_checks.py scripts/validate_research_brief_sync.py scripts/validate_research_brief_metrics.py scripts/export_research_brief_markdown.py scripts/validate_legacy_trade_batch_status.py` 通过。
- 正式重建完成：`/usr/bin/time -p` 实测 `real 20.15`，成功生成正式 `dashboard/index.html`。
- 页面解析通过：`今日 Dashboard 更新日期=2026-07-02`、`行情/回测序列最新日期=2026-07-02`、`最后回测调仓日=2026-06-29`、`预计下次回测调仓=2026-07-08`、`最后模拟盘执行日=2026-07-02`、`已落账模拟成交=2`。
- 策略监控检查通过：`signal-pill sell=0`、可见 `建议卖出=0`；本轮无新的待确认调仓。
- 本地 QA 初次因 Obsidian 项目卡片缺少最新策略结构变化行而失败；同步 `台股量化基金.md` 后重跑 `scripts/run_local_qa_checks.py` 已通过。
- 公网验证通过：Render 首页正文已切到 `2026-07-02 / 2026-07-02`，`/healthz` 为 `200`。
...
- `data/simulated_positions_2026-07-02.csv`
- `data/simulated_positions_latest.csv`
- `data/simulated_trades_2026-07-02.csv`
- `progress.md`
- `findings.md`
- `.codex/PROJECT_CONTEXT.md`
- iCloud Obsidian `台股量化基金.md`
...
执行每日收盘自动化：按 `public-close + market-mode close + multi-factor-shrink + ai_tilt moderate + --execute-simulated-trades` 口径重建 Dashboard，验证本地页面、模拟盘与研究摘要同步，并尝试推送公网。

### Actions

- 已按要求读取 automation memory、`AGENTS.md`、`task_plan.md`、`findings.md`、`progress.md`、`.codex/PROJECT_CONTEXT.md` 与 `README.md`；本轮未读取 `.env`、`.shioaji.local.env`、API key、token，未调用券商下单。
- 已执行正式重建，耗时 `real 23.88`；本轮继续带 `PYTHONPATH=$HOME/Library/Python/3.9/lib/python/site-packages` 复用 user-site `scipy`，避免 `multi-factor-shrink` 退化。
- Dashboard 今日更新日期切到 `2026-06-28`，公开收盘价共同序列仍为 `2026-06-26`；今天是周日，本轮刷新 `data/model_portfolio_market_2026-06-26.csv` 与 summary，15 檔成功、缺失 0 檔。
- 本轮 `--execute-simulated-trades` 对 `2026-06-26` 保持幂等，未新增 dated 模拟成交文件；最后模拟盘执行日仍为 `2026-06-26`，已落账模拟成交 `1` 笔：`2317` 卖出 `2` 股，执行后 `2317` 剩 `4` 股。
- 策略监控显示 `1` 笔待确认买入：`2454` 建议买入 `1` 股，触发原因为 `回落加码`；`signal-pill sell=0`、可见 `建议卖出=0`，没有已落账标的仍显示红色建议卖出。
- 已同步 Dashboard 研究摘要漂移到 QA 基线与 iCloud Obsidian `台股量化基金.md`：`AI 供应链权重 33.00%`、`风险贡献 52.82%`、`风险-权重差 +19.82%`、`trade_count=1`。

### Verification Log

- `./.venv/bin/python -m py_compile src/risk_dashboard.py scripts/serve_dashboard.py scripts/run_local_qa_checks.py scripts/validate_research_brief_sync.py scripts/validate_research_brief_metrics.py` 通过。
- 正式重建完成：`/usr/bin/time -p` 实测 `real 23.88`，成功生成正式 `dashboard/index.html`。
...
## 2026-09-23 3653 健策行情网关支持

- 本轮目标：将 3653 健策加入只读实时行情网关，使其与 Dashboard 标的池保持一致。
- 基线：`scripts/live_quotes.py` 使用 10 档硬编码列表与 r3 缓存版本；网关会校验发布 JSON 包含所有网关代码。
- 实施：名单加入 3653，缓存版本更新至 `20260923-stock-universe-r4`；不得访问凭证或调用任何交易接口。
- 验收：Python 编译通过；公网端点在 Pages 发布 11 档数据后需返回 11/11，否则不得声称部署完成。
