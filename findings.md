...
- Tony 收到的失败邮件不是公网 Dashboard 旧版，也不是 Render 发布失败；公网版本已在当天成功日更后保持正常。
- 直接原因是部署仓库 workflow 一天配置了三档 schedule；当天第一档成功提交后，后两档仍继续重建，导致在已落账后的数据状态上触发 `交易日交集少于 60`。
- 修复策略：schedule 事件先检查仓库内 `dashboard/index.html` 是否已经是今天，且行情/回测日期也为今天、待自动落账为 `0`；若满足条件，直接写 Step Summary 并跳过重建。
- 保留手动 `workflow_dispatch` 强制重建能力，避免需要人工补跑时被“已是今天”闸门挡住。
- 部署仓库修复提交为 `1442eb2`；Workspace workflow 已同步同一修改。

## 2026-07-30 16:30 漏跑补偿复核

- 2026-07-30 为台湾股市交易日；TWSE 官方 `STOCK_DAY` 公开交易资料对 `0050` 返回 `stat=OK` 且包含 `115/07/30` 成交资料。
- 16:30 复核未发现漏跑：本地 Dashboard、data 产物、Workspace 远端和公网版本均已推进到 `2026-07-30`。
- 公网 `/healthz=ok`；首页正文命中 `2026-07-30 / 2026-07-30`、`已落账模拟成交 8 笔`、`待自动落账 0 笔`；`/version.json`、首页 `ETag` 与 `X-Dashboard-Version` 一致为 `442a8c27c8a50974d06edf7d95d6fa74f0e3c02e71a5c2fb03e8f5222b4463a8`。
- 本轮属于复核通过，不需要补偿重建、模拟盘落账或推送部署仓库；仅提交并推送 Workspace 交接记录，未触碰敏感配置或券商交易端。

## 2026-07-30 收盘日更

- 2026-07-30 为台湾股市交易日；TWSE 官方 2026 年市场开休市资料未列为休市，且 13:59 已确认重点标的公开月资料补到 `115/07/30`。
- 收盘后资料仍有分批状态：13:46 重点标的多停在 `115/07/29`，本轮等待公开资料补齐后才重建，避免 Dashboard 生成日与行情/回测共同日错位。
...
- 2026-07-29 為台灣股市交易日；TWSE 官方 2026 年市場開休市資料未列為休市，且 14:04 已確認重點標的公開月資料補到 `115/07/29`。
- 收盤後資料發布有短暫分批狀態：13:47 至 14:00 期間部分標的仍停在 `115/07/28`，本輪先等待公開資料補齊，避免把 Dashboard 更新日推進但行情/回測日留在舊日。
...
- GitHub Actions 今天实际触发 3 次；唯一作业均在 `Rebuild dashboard` 退出，邮件里的 `All jobs have failed` 只是汇总提示。
- 直接根因是工作流把云端任务设成 `--offline-cache`，但行情缓存目录不进 Git，只由 Actions cache 提供。缓存未命中时，首次 `load_prices` 没有任何可用资产行情；作业失败后缓存也不会进入可复用的成功收尾。
- 本机已有本地缓存，所以同一正式口径能成功推进到 `2026-07-28`；模型与页面生成代码不是本轮根因。
- 修复口径：云端任务直接允许 TWSE 公开收盘抓取并保留 `--allow-stale-cache` 作为接口异常时的降级；另加 GitHub Step Summary，输出交易日判定、重建和页面验收状态。页面日期或行情日期不等于当天时仍硬失败，避免“任务成功但页面没更新”。
...
- Dashboard “每日更新 Summary” 需要成为日常报告面，不只记录更新流水，还要每天汇总量化模型基金组合的绩效与 KPI：模型盘 P&L、滚动回测净值、年化报酬、最大回撤、年化波动、换手、AI 风险差与模拟盘执行状态。
- 正常日更/复核的详细摘要应进入 Dashboard、项目交接文档和 automation memory；本地线程不再复制完整“复核通过摘要”“复核跳过摘要”或“漏跑补偿摘要”。
...
- 已修复 Dashboard 信息密度：每日摘要上移为首屏 `今日状态`，手机端行情与状态卡片两列展示，资产池改为横向 chip；调仓日历改为 6 项 timeline，不再用大空框显示少量文字。
- “执行检查/执行复核”类文案会让用户误解为仍需人工批准；当前按钮与折叠区统一为“模拟盘状态”，CSV 落账仍由收盘脚本自动完成，页面只做浏览器审计。
...
- 2026-07-27 用户明确首屏应围绕基金状态；内部运行标签移出主视线，研究图表、回测、组合明细和数据问题改为原生折叠区。
- “模拟盘现金”采用初始虚拟资金减当前持仓成本基础的模型盘估算，并明确标注为模拟盘，不冒充券商账户余额。

## 2026-09-23 3653 健策行情完整性

- 行情网关和 Dashboard 都有标的代码完整性白名单。只改静态公开 JSON 或前端，会让 3653 在实时刷新时缺档或被当成不完整结果；三个清单和 r4 缓存版本须同步。
- Shioaji 与 TWSE MIS 使用只读快照；不得读取部署凭证或触碰下单接口。
