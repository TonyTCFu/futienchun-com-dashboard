...
- The primary cloud automation path is `.github/workflows/tw-dashboard-daily.yml` in the deployed Dashboard repository. It runs on GitHub Actions after Taiwan close, gates on the TWSE official holiday calendar, rebuilds public-close data on trading days, validates the generated Dashboard, commits generated artifacts, and pushes to trigger Render deployment. Local Codex cron remains a supplemental audit/compensation path, not the only production scheduler.
- If the Taiwan equity market is open on a given date, the Dashboard update is mandatory for that trading day: rebuild, local paper-ledger artifacts, Workspace push, public deployment push, and public cache-version verification must complete. Only weekends and official exchange holidays may skip the rebuild.
...
- Daily and watchdog automations must treat the public Dashboard as the user-facing report surface. Normal successful daily-close, non-trading-day, review, and miss-compensation results should not be posted back into the project Codex thread; only errors, blockers, stale public versions, failed publishing, failed verification, or user-decision needs should create a thread message. Detailed daily / review summaries go to the Dashboard daily update summary, handoff docs, and automation memory.
- The Dashboard daily update summary must include a portfolio KPI section for the model-fund paper portfolio: model-portfolio P&L, rolling backtest net value, annual return, max drawdown, volatility, turnover, AI risk gap, and paper-trade execution status compared against the plain covariance or policy baseline where available.
...
- `--execute-simulated-trades` must ledger both buy and sell signals in the local paper portfolio when cash/position constraints allow it. User-visible Dashboard wording should say automatic ledgering / browser audit, not imply that Tony must manually approve a pending rebalance before CSV execution.
- The Dashboard must not show large `暂不可用` cards when only the standalone TWSE TAIEX index endpoint fails. If the model-portfolio market values exist, use portfolio market value, unrealized P&L, and simulated-trade status as the fallback summary; Shioaji/public-close holding prices remain the useful report surface.
- The Dashboard first viewport should prioritize actionable status over explanatory copy. Keep `今日状态` compact, use side-by-side metric grids/timelines where possible, avoid large cards with only a few words, and do not use `执行检查/执行复核/待复核` wording for automatic paper-ledger flows.
...
- 2026-07-27: Tony reported another trading-day miss. Treat local Codex automations as unreliable for production scheduling because they depend on the local desktop execution layer. The durable fix is a cloud scheduler in the deployment repository; local automation should only supplement cloud verification and handoff sync.
